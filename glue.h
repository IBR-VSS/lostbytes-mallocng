...
static volatile int malloc_active = 0;

static inline int a_ctz_32(uint32_t x) { return __builtin_ctz(x); }

static inline int a_clz_32(uint32_t x) { return __builtin_clz(x); }
...
static inline void a_or(volatile int *p, int v) { __sync_fetch_and_or(p, v); }
...
__attribute__((__visibility__("hidden"))) extern pthread_mutex_t malloc_lock;

#define LOCK_OBJ_DEF pthread_mutex_t malloc_lock = PTHREAD_MUTEX_INITIALIZER
...
__attribute__((__visibility__("hidden"))) extern pthread_rwlock_t malloc_lock;

#define LOCK_OBJ_DEF pthread_rwlock_t malloc_lock = PTHREAD_RWLOCK_INITIALIZER
